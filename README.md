...
> Generated from `src/frontend/` by CI and deployed to GitHub Pages. A demo shim acts as a fake backend, so you can try page transitions, slide controls, and LIVE/SLOW/OFFLINE network states without a server; every page also works when opened directly.

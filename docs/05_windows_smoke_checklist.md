...
`finish/` を空にする。項目 12（同名 collision）を検証する回に限り、テスト用 `.pptx` のうち 1 枚と同名のファイルを `finish/` 側にも置いておく。角括弧ケースも見る回は、対象フォルダに `deck[1].pptx` を、`finish/` に `deck1.pptx` を置いておく（移動後に `deck[1].pptx` がそのまま入り、`deck1.pptx` が無傷であることを確認）。
...
| 8 | NowPlaying（2 端末） | lock on / off / steal が機能する。非 owner 端末の `/slide/*` 操作はサーバ側で拒否される。`/stop` は認証済みなら任意端末（非 owner でも）が停止できる＝緊急停止の仕様（PR-E / 案B 確定・意図的） | ☐ |

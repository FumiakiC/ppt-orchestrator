...
8. adapter/IP 表示改善。
9. GitHub Pages UI モックアップの保守（製品側 HTTP 契約を変えた際の shim 追従。✅ 初期実装は PR-L / PR-M で完了）。
...
| モックアップ生成 | `build-mockup.ps1` の実行成功（8 ページ + `.nojekyll`）、`%%TOKEN%%` 残骸ゼロ、`</head>` 欠落時に fail fast すること（改訂 9 / PR-L・PR-M） |
...
- モックアップ shim（`build/mockup/demo-shim.js`）は製品側の HTTP 契約に依存するが、その追従は CI では検証できない。route / `/status` の返す文字列 / `/slide/state` の JSON 形状を変える PR では、レビュー時に人手で追従を確認する必要がある（自動検証の要否は将来判断）。

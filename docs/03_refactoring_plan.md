...
| `.bat` 末尾 | **事実修正（改訂 1）**: 残っているのは「``` 行」ではなく、`exit /b 0` の後の**バッククォート 2 文字 `` ` ` ``（終端改行なし）**。`exit /b 0` 後のため実害はないが除去する。 | P0 |
| UPN 抽出 | **機序確認済み（改訂 1）**: `whoami /upn` は UPN 文字列（例 `user@domain.com`）のみを出力し `:` を含まないため、`find ":"` は**常に不一致**となり `CURRENT_UPN` は決して設定されない（= 100% fallback）。仮に一致しても `delims=:` の `%%B` 抽出で値が壊れる二重の欠陥。`find` を外し `whoami /upn` の出力を直接取得する形に修正する。URLACL 登録ユーザーの正確性に影響する。 | P1 |
| src エンコーディング不統一 | **新規（改訂 1）**: `com-handler.ps1` と `ui-console.ps1` は BOM なし UTF-8 + LF、他の `src/*.ps1` は BOM 付き UTF-8 + CRLF。`build.ps1` が `-Encoding UTF8` で読み BOM 付きで `dist` を書くため**製品配布物は無害**だが、Windows PowerShell 5.1 で `src` を直接実行すると BOM なしファイルは ANSI 解釈され日本語コメント・文字列が化ける。`.gitattributes` / `.editorconfig` で統一する。 | P2 |
...
4. `Start-Presenter.bat` 末尾の不要なバッククォート 2 文字の削除。未着手。PR-B で次着手。
...
12. `finish/` 移動が正しい。⚠ 現状の同名 collision は `Move-ToFinishIfPending` が `Move-Item -Force` を使用するため上書きであり、既存ファイルは失われる。「既存ファイルを消さない」は PR-F 完了後の期待値である。PR-F マージ後に本項目と `docs/05_windows_smoke_checklist.md` の期待値を更新する。

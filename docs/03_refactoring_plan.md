...
5. `.bat` 末尾の不要なバッククォート 2 文字を削除する。
6. ログ方針、security header 方針、token 方針を docs に記録する。
7. `.gitattributes` / `.editorconfig` を追加し、`src/*.ps1` を BOM 付き UTF-8 + CRLF に統一する（`dist` 生成物は build 側で既に BOM 付き固定のため挙動不変）。
...
4. `Start-Presenter.bat` 末尾の不要なバッククォート 2 文字の削除。

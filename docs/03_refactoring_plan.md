...
| `build/build-mockup.ps1` | **CI 限定**。`src/frontend/**` から静的 UI モックアップ（`dist/pages/`）を生成。実行時の組み立てを模倣し、demo shim を `</head>` 直前に注入する。製品 `dist` には非同梱 |
| `build/mockup/demo-shim.js` | **CI 限定**。Pages 専用の疑似バックエンド。`fetch` と form 送信を横取りし `/status` / `/slide/*` / `/lock/*` を模擬する。製品 JS は無改変のまま実走させる |
